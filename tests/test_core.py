...
def test_ls_alive_only():
    core.register("alive-test", alias="alive")
...
def test_heartbeat():
    ...
    reg_before = json.loads((core.REGISTRY_DIR / "hb-test.json").read_text())
    ...
    reg_after = json.loads((core.REGISTRY_DIR / "hb-test.json").read_text())
    assert reg_after["heartbeat"] >= reg_before["heartbeat"]
